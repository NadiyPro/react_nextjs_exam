...
const MovieSearchPage = async ({searchParams: {query}}:ISearchProps) => {
    const search:ISearchPageResult[] | string = query? await getSearchPage(query) as ISearchPageResult[] : 'There are no search results for your request';
    ...
    return (
        <div>
            ...
            <div>
                ...
                {/*    <Link href={`/search/movie?query=${query}&page=${page > 1 ? page - 1 : 1}`}>Prev {page > 1 ? page - 1 : 1}</Link>*/}
                {/*</button>*/}
                {/*{page}*/}
                {/*<button className={style.button_pagination}>*/}
                {/*    <Link href={`/search/movie?query=${query}&page=${page < 1 ? 1 : +page + 1}`}>Next {page < 1 ? 1 : +page + 1}</Link>*/}
                {/*</button>*/}
                {/*<button className={style.button_pagination}>*/}
